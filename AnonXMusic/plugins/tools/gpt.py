...
async def chat_arvis(app, message):
    try:
        ...
        if len(message.command) < 2:
            await message.reply_text(f"Hᴇʏ {nane} I ᴀᴍ ʜᴇʀᴇ!💪 Wʜᴇʀᴇ's ᴛʜᴇ ᴇɴᴇᴍʏ 🧟‍♂")
        else:
...
async def chat_gpt(app, message):
    try:
        ...
        if len(message.command) < 2:
            await message.reply_text("Hᴇʏ {name} I ᴀᴍ ʜᴇʀᴇ ᴛᴏ ᴀssɪsᴛ ʏᴏᴜ! , Tᴇʟʟ ᴍᴇ ᴡʜᴀᴛ ᴅᴏ ʏᴏᴜ ᴡᴀɴɴᴀ ᴋɴᴏᴡ")
        else:")
        else:
            query = message.text.split(' ', 1)[1]
            MODEL = "gpt-3.5-turbo"
